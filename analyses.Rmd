...
We created a data subset to test the demographic, morphometric and behavioural variables for the first assay for all 36 individual eastern quolls. 
...
We created a data subset to test the behavioural variables over multiple assays for 17 individual eastern quolls. 
...
NB - We did not test 'stress in trap' in the previous chunk as there was no variation between individual stress ratings. 
...
We minimised environmental influence on the behaviour models by standardising their values: the means of the environmental variables were all scaled to zero, and the standard deviation to one, as in Wilson *et al*. (2022).
...
We fitted GLMs of the adjusted behaviours against assay number and the standardised environmental variables, and at the same time, we extracted intercept (‘personality’ measure) and slope (‘plasticity’ measure) coefficients for each behavioural variable. 
...
We created data frame for personality and plasticity analysis with individual eastern quoll demographic and morphological traits.  
...
Using the adjusted values we fitted models to extract the intercept (personality) and slope (plasticity) over multiple assays. 
...
We tested competing models using Akaike Information Criteria, corrected for small samples sizes model selection test. 


```{r}
# MOdel selection for slope of occurrences of jumping
mod_jos_site <- lmer(s_jumpo_p ~ site + (1|arena), data=ppdata, 
                     control=control_ml, na.action="na.fail", REML=FALSE)

mod_jos_weight <- lmer(s_jumpo_p ~ sex*weight + (1|arena), data=ppdata, 
                       control=control_ml, na.action="na.fail", REML=FALSE)
# 
...
# Model selection for slope of duration spent on hind legs
mod_los_site <- lmer(s_legso_p ~ site + (1|arena), data=ppdata, 
                     control=control_ml, na.action="na.fail", REML=FALSE)

mod_los_weight <- lmer(s_legso_p ~ sex*weight + (1|arena), data=ppdata, 
                       control=control_ml, na.action="na.fail", REML=FALSE)
# 
...
```{r}
# Sum of section visits (standalone plot)
i_sumsect_site <- ggplot(data=ppdata, aes(x=site, y=i_sumsecto_p, fill=site)) + 
  geom_violin(alpha=0.8) + 
  geom_boxplot(col=steel, fill=steel, width=0.02, lwd=0.5) + 
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x=0.9, y=70, label=expression(italic(p) == 0.005), size=5) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
        ...
        axis.text.x=element_text(size=19), 
        plot.margin=margin(0.5, 0.5, 0.5, 1, "cm")) + 
  scale_fill_manual (values=c(vir3, vir9)) + 
...
i_bs_site <- ggplot(data=ppdata, aes(x=site, y=i_stress_bag_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=1.3, label="a", size=10, hjust=-2) + 
  annotate("text", x=1, y=1.3, label=expression(italic(p) == "<0.001"), size=8) +
theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
        ...
        plot.margin=margin(t=0.5, r=0.5, b=0.5, l=0.5, "cm")) + 
  scale_fill_manual (values=c(vir3, vir9)) + 
  #scale_x_discrete(labels=c("Goorooyarroo"="GS", "Mulligans Flat"="MFWS")) +
scale_y_continuous(breaks=seq(0.9, 1.3, 0.1), 
                     labels=seq(0.9, 1.3, 0.1), 
                     limits=c(0.9, 1.3)) + 
  labs("") + xlab("") + ylab("Bag stress rating") 
...
ggsave(filename="output/plots/intercept/study site by bag stress.jpeg", 
       i_bs_site, width=100, height=100, units="mm")
```
...
i_ts_site <- ggplot(data=ppdata, aes(x=site, y=i_stress_table_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=1.3, label="b", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=1.3, label=expression(italic(p) == "<0.001"), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
  ...
  scale_y_continuous(breaks=seq(0.9, 1.3, 0.1), 
                     labels=seq(0.9, 1.3, 0.1), 
                     limits=c(0.9, 1.3)) + 
  labs("") + xlab("") + ylab("Table stress rating") 
...
ggsave(filename="output/plots/intercept/study site by table stress.jpeg", 
       i_ts_site, width=100, height=100, units="mm")
```
...
i_e_site <- ggplot(data=ppdata, aes(x=site, y=i_emerge_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=50, label="c", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=50, label=expression(italic(p) == "<0.001"), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
  ...
  scale_y_continuous(breaks=seq(0, 50, 10), 
                     labels=seq(0, 50, 10), 
                     limits=c(0, 50)) + 
  labs("") + xlab("") + ylab("Latency to emerge (s)") 
...
i_md_site <- ggplot(data=ppdata, aes(x=site, y=i_moved_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=200, label="d", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=200, label=expression(italic(p) == 0.017), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
...
i_ld_site <- ggplot(data=ppdata, aes(x=site, y=i_legsd_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=300, label="e", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=300, label=expression(italic(p) == "<0.001"), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
  ...
  scale_fill_manual (values=c(vir3, vir9)) + 
  labs("") + xlab("") +
  ylab("Hindleg stands duration (s)") 
...
i_jo_site <- ggplot(data=ppdata, aes(x=site, y=i_jumpo_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=100, label="f", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=100, label=expression(italic(p) == "<0.001"), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
  ...
  scale_fill_manual (values=c(vir3, vir9)) + 
  labs("") + xlab("") + 
  ylab(expression(paste("Jumping + climbing occurrences ", italic("(n)")))) 
...
i_jd_site <- ggplot(data=ppdata, aes(x=site, y=i_jumpd_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=250, label="g", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=250, label=expression(italic(p) == "<0.001"), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
  ...
  labs("") + xlab("") +
  scale_x_discrete(labels=c("Goorooyarroo"="GS", "Mulligans Flat"="MFWS")) +
  ylab("Jumping + climbing duration (s)") 
...
i_pd_site <- ggplot(data=ppdata, aes(x=site, y=i_perchd_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=300, label="h", size=10, hjust=-2) + 
  annotate("text", x=0.9, y=300, label=expression(italic(p) == 0.017), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
...
i_po_site <- ggplot(data=ppdata, aes(x=site, y=i_percho_p, fill=site)) + 
  ...
  stat_summary(fun="median", geom="point", col=black, fill=white, 
               shape=21, size=3, stroke=1) + 
  annotate("text", x="Mulligans Flat", y=30, label="i", size=10, hjust=-7) + 
  annotate("text", x=0.9, y=30, label=expression(italic(p) == "<0.001"), size=8) +
  theme_minimal() + 
  theme(legend.position="none", 
        panel.grid=element_blank(), 
        axis.line.x=element_line(colour=steel), 
  ...
  scale_x_discrete(labels=c("Goorooyarroo"="GS", "Mulligans Flat"="MFWS")) +
  xlab("") + 
  ylab(expression(paste("Perching occurrences ", 
                        italic("(n)")))) 
...
```{r}
# Combine plots
plot <- ggarrange(i_bs_site, i_ts_site, i_e_site, i_md_site,i_ld_site, i_jo_site, i_jd_site, i_pd_site, i_po_site, nrow=3, ncol=3)

# Annotate plot
plot <- annotate_figure(plot, bottom=text_grob("Study site", size=25), left=text_grob("Adjusted intercept values", size=25, rot=90))
...
ggsave(filename="output/plots/intercept/combined intercept plot.jpeg", 
       plot, width=400, height=400, units="mm")
```
