...
pacman::p_load(beepr, broom, broom.mixed, dplyr, grid, gtools, janitor, 
               lubridate, readxl, sjmisc, sjlabelled, 
               tidyverse, writexl, # General functionality
               ggplot2, ggpubr, sjPlot, scales, viridis, # Plotting
               ggmap, sf, # Mapping
...
# Assign repeated values and constants to objects
...
groomd_site <- ggplot(data=data1, aes(x=site, y=grooming_d, fill=site)) + 
  ...
  geom_boxplot(col=grey, fill=grey, width=0.02, lwd=0.5) + 
...
```{r}
# 
summary(lmer(i_bait_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_emerge_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_foodd_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_foodo_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_groomd_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))
        
summary(lmer(i_groomo_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_jumpd_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_jumpo_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_legsd_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_legso_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_moved_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_moveo_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_perchd_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_percho_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_sumsecto_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_totalacto_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_stress_bag_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))

summary(lmer(i_stress_table_p ~ sex + weight + site + cond_n + age_n + 
               (1|arena), data=ppdata, na.action="na.fail", REML=FALSE))
```

NB - I did not test 'stress in trap' in the previous chunk as there was no variation between individual stress ratings. 

# p value adjustment fiddling
...
# Define predictors
predictors <- ppdata %>% 
  select(age_n, weight, cond_n, sex, study_site) %>%
  colnames()

# Run lmer models across all responses
lmer_results <- map_dfr(responses, function(beh) {
  
  # Build formula string
  formula_str <- paste(beh, "~", paste(predictors, collapse=" + "))
  ...
  # Fit model only if there's enough data
  if(nrow(model_data) >= length(predictors) + 1) {
    model <- glm(as.formula(formula_str), data=ppdata, na.action=na.fail)
    
    tidy(model) %>%
      filter(term %in% predictors) %>%
      mutate(behaviour=beh)
  
  # If model fails (e.g., not enough data), return empty tibble
  } else {tibble()}})

# Apply FDR correction across all predictor-behaviour tests
lmer_results <- lmer_results %>%
  mutate(p_adj=p.adjust(p.value, method="BH")) %>%  # Benjamini-Hochberg 
  mutate(significant=p_adj < 0.05) 

# View top results
lmer_results %>%
  select(behaviour, term, estimate, p.value, p_adj, significant)
