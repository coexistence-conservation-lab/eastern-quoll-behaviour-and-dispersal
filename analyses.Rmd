...
author: "Christina Gee and Belinda Wilson"
date: "March 4, 2025"
output: 
  html_document:
    toc: true
...
Here we describe the analyses conducted for the journal article Gee *et al*. (in prep) "*Ranging after reintroduction: do behavioural traits predict dispersal into new territory?*". We conducted field-based behavioural assays on eastern quolls (*Dasyurus viverrinus*) from two self-selecting populations:

  1. An original safe haven where the species was reintroduced, and 
  2. A new, adjacent haven that eastern quolls could disperse to but not leave. 
  
We aimed to determine whether rapid field assays could detect behavioural differences between these populations, and whether these differences influence these populations’ dynamics and traits.
...
# **Definitions**
  ...
  - **GLMM:** Generalised linear mixed model (i.e., with random effects)
  - **_i:** Intercept
...
Here we install [pacman](https://cran.r-project.org/web/packages/pacman/index.html) (enables efficient installation and loading of other packages). Note that this installation only needs to be done once per computer.
...
```
 ...
 2. Install other required packages using the `pacman` package.

```{r, results='hide', warning=FALSE, message=FALSE}
# Install and load required packages
pacman::p_load(AICcmodavg, beepr, broom, broom.mixed, corrplot, dplyr, ggplot2, 
               ggpubr, grid, gtools, janitor, jPlot, kableExtra, lme4, lmerTest, 
               lubridate, MuMIn, readr, readxl, rstudioapi, scales, sf, sjmisc, 
               sjlabelled, tidyr, tidyverse, viridis, writexl)
```
...
# Export plot as a jpeg file
ggsave(filename="output/plots/plasticity plots.jpeg", 
       plot, width=450, height=700, units="mm")
```
