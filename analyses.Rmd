...
table <- tidy_results %>%
  mutate(p.value = ifelse(p.value <= 0.05, cell_spec(p.value, bold = TRUE), p.value)) %>%
  kbl(caption = "TTotal activity occurrences <em>(n)</em>", escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Times New Roman") %>%
  kable_styling(bootstrap_options = "striped", font_size = 15)
table
...
#mod1 <- lmer(foodo_pred ~ study_site + sex + age_estimate + condition +  body_weight_kg + morph + ssr + pes_mean_mm + head_length_mm + (1|quoll_code) + (1|arena), 
...
mod1 <- lmer(emerge_pred ~ study_site + sex + age_estimate + condition + 
              body_weight_kg + morph + pes_mean_mm + head_length_mm + (1|arena), 
            data=data1_pred, na.action = "na.fail")

mod2 <- lmer(foodo_pred ~ study_site + sex + age_estimate + condition + 
              body_weight_kg + morph + pes_mean_mm + head_length_mm + (1|arena), 
            data=data1_pred, na.action = "na.fail")
