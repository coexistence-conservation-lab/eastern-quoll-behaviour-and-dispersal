...
print(plot)
ggsave(filename = "intercept plots.jpeg", plot, width=400, height= 600, units = "mm")
...
s_fd_site <- ggplot(data=ppdata, aes(x = study_site, y = s_foodd_p, fill=study_site)) + 
 geom_violin() +
  ...
  stat_summary(fun="median", geom="point", col="black", fill="white", 
               shape=21, size=3, stroke=1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +  
  scale_fill_manual (values = c("#2D718E","#B8DE29")) +
  theme_minimal() +
  theme(legend.position = "none", 
        panel.grid = element_blank(),
        axis.line.x = element_line(color = "black"),
  ...
  xlab("") + 
  ylab("Adjusted food \ninteraction duration (s) slope") + labs("") +
...
food <- ggplot(data=d2pred, aes(x = assay_number, y =foodd_p, color = study_site, fill = study_site)) + 
 geom_point() +
  ...
  annotate("text", x =max(d2pred$assay_number), y =110, label = "B", size = 10, hjust = 2)
