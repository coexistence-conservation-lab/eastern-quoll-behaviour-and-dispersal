...
data2_mutate <- data2 %>%
  mutate_at(vars(5, 44, 65, 65), ~ as.vector(scale(.)))
...
# Write to csv file
write_csv(ppdata, "pp data - intercept and slope data.csv")
...
#Testing behaviour intercept against independent variables using GLMMs
```{r}
names(ppdata)
...
table <- tidy_results %>%
  mutate(p.value = ifelse(p.value <= 0.05, cell_spec(p.value, bold = TRUE), p.value)) %>%
  kbl(caption = "Table stress rating intercept", escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Times New Roman") %>%
  kable_styling(bootstrap_options = "striped", font_size = 12)
table
...
COPIED FROM 1ST ASSAY - REWRITE FOR 2 
