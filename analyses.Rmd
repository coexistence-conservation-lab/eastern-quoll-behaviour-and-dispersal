...
jpeg(height=2000, width=2000, file="output/plots/correlation plot.jpeg", 
     type="cairo")
corrplot(cor, method="color", col=colorRampPalette(c("#B8DE29", "#FFFFFF", 
                                                     "#2D718E"))(10), 
...
mod <- lmer(bait_eaten_g ~ study_site + sex + age_n + 
            ...
            data=data1, na.action="na.fail", REML=FALSE)
...
mod_gd_site <- lmer(grooming_d ~ study_site + (1|arena), 
            data=data1, control=control_ml, na.action="na.fail", REML=FALSE)
...
mod <- lmer(bait_eaten_g ~ study_site + sex + age_n + 
            ...
            data=data2, na.action="na.fail", REML=FALSE)
...
mod_fo_process <- lmer(food_o ~ time_processtoassay + (1|arena), 
            data=data2, control=control_ml, na.action="na.fail", REML=FALSE)
...
# Export plot as a jpeg file
ggsave(filename="output/plots/2 assay raw - moved light.jpeg", 
...
# Export plot as a jpeg file
ggsave(filename="output/plots/2 assay raw - move occ light.jpeg", 
...
# Export plot as a jpeg file
ggsave(filename="output/plots/2 assay raw - move occ process.jpeg", 
...
ed_plot <- ggplot(data=data2_pred, aes(x=assay_number, y=emerge_p, 
  ...
  annotate("text", x="2", y=50, label="D", size=10, hjust=-3)
...
mod <- lmer(i_bait_p ~ sex + body_weight_kg + study_site + 
            ...
            data=ppdata, na.action="na.fail", REML=FALSE)
...
mod_pdi_site <- lmer(i_perchd_p ~ study_site + (1|arena), 
            data=ppdata, control=control_ml, na.action="na.fail", REML=FALSE)
