...
data$sumsect_o <- rowSums(data[, c("section_1_o", "section_2_o", "section_3_o", "section_4_o")])

#Adding together all activity without sections
data$totalact_o <- rowSums(data[, c(21:27)])
...
mov <- ggplot(data, aes(x = as.character(assay_number), y = moving_d, 
...
movo <- ggplot(data, aes(x = as.character(assay_number), y = moving_o, 
                        fill = as.character(assay_number))) + 
  geom_violin() +
  facet_wrap(~study_site) + 
  theme_minimal() +
...
summary(glm(moving_d ~ study_site + temp + assay_number, data=data))
summary(glm(moving_o ~ study_site + temp + assay_number, data=data))
...
*Look in Belinda's behaviour code*

```{r}

#Finding out the locations of the relevant columns 
colfoodo <- which(colnames(data) == "food_o")
print(colfoodo)

restingo <- which(colnames(data) == "resting_o")
print(restingo)

colsection4o <- which(colnames(sexdata) == "section_4_o")
print(colsection4o)

colfoodd <- which(colnames(sexdata) == "food_d")
print(colfoodd)

colsection4d <- which(colnames(sexdata) == "section_4_d")
print(colsection4d)
...
#Running the correlations tests for both, then occurrences, then duration
